import { IconX, IconMinus, IconPlus, IconCheck, IconAlertTriangle, IconArrowRight, IconBuildingWarehouse, IconChevronDown, IconFilter } from '@tabler/icons-react'
import VariantChips from './VariantChips'
import { fmt } from './format'

export default function DistribuirStockPanel({
  art, dep, onOpenDepPanel, onOpenFiltroPanel, depStock,
  distFiltros,
  distRows, distAsig, setAsig, distStockBase, distTotal, distRestante, distOver, distHasChanges,
  onConfirm, onClose,
}) {
  const hasDistFiltros = Object.values(distFiltros).some((v) => v.length > 0)

  return (
    <div className="va-card vg-dist-tab">
      <div className="va-panel-head">
        <div className="va-ico"><IconArrowRight size={18} stroke={1.6} /></div>
        <div className="va-grow">
          <div className="va-eyebrow">{art.codigo}</div>
          <div className="va-title">Distribuir stock</div>
        </div>
        <button type="button" className="va-btn-icon va-close" onClick={onClose} aria-label="Cerrar solapa" title="Cerrar solapa">
          <IconX size={18} stroke={1.6} />
        </button>
      </div>

      <div className="vg-flow-body">
        <div className="st-filter-row" style={{ marginBottom: 0 }}>
          <button type="button" className="st-filter-btn" onClick={onOpenDepPanel}>
            <IconBuildingWarehouse size={14} stroke={1.6} /> {dep ? dep.nombre : 'Elegir depósito'} <IconChevronDown size={12} stroke={1.8} />
          </button>
          {art.groupers.length > 0 && (
            <button type="button" className={`st-filter-btn ${hasDistFiltros ? 'is-active' : ''}`} onClick={onOpenFiltroPanel}>
              <IconFilter size={14} stroke={1.6} /> Variantes
              {hasDistFiltros && <span className="st-filter-dot" />}
              <IconChevronDown size={12} stroke={1.8} />
            </button>
          )}
        </div>

        <div className="st-kpi-row" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
          <div className="st-kpi-card st-kpi-total">
            <div className="st-kpi-label">Disponible</div>
            <div className="st-kpi-value">{fmt(distStockBase)}</div>
            <div className="st-kpi-sub">Stock base sin asignar en {dep ? dep.nombre : 'el depósito'}</div>
          </div>
          <div className="st-kpi-card st-kpi-acopiado">
            <div className="st-kpi-label">Asignado</div>
            <div className="st-kpi-value">{fmt(distTotal)}</div>
            <div className="st-kpi-sub">En esta distribución</div>
          </div>
          <div className={`st-kpi-card ${distOver ? 'st-kpi-comprometido' : 'st-kpi-disponible'}`}>
            <div className="st-kpi-label">{distOver ? 'Excedente' : 'Sin asignar'}</div>
            <div className="st-kpi-value">{fmt(Math.abs(distRestante))}</div>
            <div className="st-kpi-sub">{distOver ? 'Ajustá las cantidades' : 'Del stock disponible'}</div>
          </div>
        </div>

        {distOver && (
          <div className="vg-alert vg-alert-danger">
            <IconAlertTriangle size={15} stroke={1.9} />
            Estás asignando {fmt(Math.abs(distRestante))} unidades más que las disponibles en el artículo base. Ajustá las cantidades para confirmar.
          </div>
        )}
        {distStockBase === 0 && !distOver && (
          <div className="vg-alert vg-alert-warn">
            <IconAlertTriangle size={15} stroke={1.9} />
            Este artículo base no tiene stock por asignar en {dep ? dep.nombre : 'este depósito'}. Ingresá stock o elegí otro depósito.
          </div>
        )}

        <div className="va-card st-table-card">
          <div className="va-card-scroll">
            <table className="va-grid">
              <thead>
                <tr>
                  <th>Variante</th>
                  <th>Código</th>
                  <th style={{ textAlign: 'right' }}>Stock actual</th>
                  <th style={{ textAlign: 'center', width: 180 }}>A asignar</th>
                  <th style={{ textAlign: 'right' }}>Stock resultante</th>
                </tr>
              </thead>
              <tbody>
                {distRows.map((v) => {
                  const a = Number(distAsig[v.id] || 0)
                  const stockActual = depStock(v)
                  return (
                    <tr key={v.id} className={a > 0 ? 'is-selected' : a < 0 ? 'is-return' : ''}>
                      <td><VariantChips groupers={art.groupers} vals={v.vals} /></td>
                      <td className="pr-cell-muted vg-mono">{v.codigo}</td>
                      <td className="st-num" style={{ textAlign: 'right' }}>{fmt(stockActual)}</td>
                      <td>
                        <div className="vg-stepper">
                          <button type="button" className="vg-stepper-btn" onClick={() => setAsig(v, a - 1)}>
                            <IconMinus size={13} stroke={2.4} />
                          </button>
                          <input
                            value={a}
                            onChange={(e) => setAsig(v, e.target.value)}
                            className={`vg-stepper-input ${a < 0 ? 'is-negative' : ''}`}
                          />
                          <button type="button" className="vg-stepper-btn" onClick={() => setAsig(v, a + 1)}>
                            <IconPlus size={13} stroke={2.4} />
                          </button>
                        </div>
                      </td>
                      <td className="st-saldo" style={{ textAlign: 'right' }}>{fmt(stockActual + a)}</td>
                    </tr>
                  )
                })}
                {distRows.length === 0 && (
                  <tr><td colSpan={5} className="va-empty-cell">Ninguna variante generada coincide con los filtros. Generá variantes o quitá filtros para asignar stock.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        </div>
      </div>

      <div className="va-panel-foot">
        <span className="pr-foot-status">
          {!distHasChanges
            ? 'Cargá cantidades para distribuir el stock base.'
            : distTotal > 0
              ? `Se asignarán ${fmt(distTotal)} de ${fmt(distStockBase)} unidades en ${dep ? dep.nombre : 'el depósito seleccionado'}.`
              : distTotal < 0
                ? `Se devolverán ${fmt(Math.abs(distTotal))} unidades al artículo base en ${dep ? dep.nombre : 'el depósito seleccionado'}.`
                : 'Se moverán unidades entre variantes sin afectar el stock base.'}
        </span>
        <button type="button" className="va-btn va-btn-secondary" onClick={onClose}>Cancelar</button>
        <button type="button" className="va-btn va-btn-primary" disabled={!distHasChanges || distOver} onClick={onConfirm}>
          <IconCheck size={15} stroke={2} /> {distTotal > 0 ? 'Asignar ubicaciones' : 'Confirmar'}
        </button>
      </div>
    </div>
  )
}
